...
var rules = function rules(ruleName) {
  return {
    valid: function valid(item) {
      var validation = validations[ruleName];
...
var validateField = function validateField(item) {
  var errors = [];
  Object.keys(item.validate).forEach(function (validation) {
    var rule = rules(validation);
...
function useSimpleFormState(config) {
  ...
  var _validateItem = function _validateItem(name, state) {
    var item = lodash.get(state, name);
